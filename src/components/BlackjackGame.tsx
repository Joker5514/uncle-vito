...
const BlackjackGame: React.FC<{ setVitoMessage: (msg: VitoMessage) => void }> = ({ setVitoMessage }) => {
  ...
  // ⚡ Bolt Optimization: Calculate derived state during render
  // What: Removed useState and useEffect for playerScore and dealerScore
  // Why: Storing derived values in state using useEffect causes an extra, unnecessary re-render cycle after the hand state updates
  // Impact: Reduces re-renders by ~50% per card drawn/dealt and improves visual responsiveness

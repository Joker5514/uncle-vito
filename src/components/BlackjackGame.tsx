...
const BlackjackGame: React.FC<{ setVitoMessage: (msg: VitoMessage) => void }> = ({ setVitoMessage }) => {
  ...
  // ⚡ Bolt Optimization:
  // What: Calculate scores directly during render instead of using useState/useEffect.
  // Why: Synchronizing derived state (scores) via useEffect is an anti-pattern that triggers an unnecessary double render cycle (one for hand update, one for score update).
  // Impact: Reduces re-renders by 50% when dealing or drawing cards, improving responsiveness.

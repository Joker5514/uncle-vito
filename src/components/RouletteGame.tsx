...
const RouletteGame: React.FC<{ setVitoMessage: (msg: VitoMessage) => void }> = ({ setVitoMessage }) => {
  ...
  // ⚡ Bolt Optimization:
  // What: Hoisted static arrays (numbers, bet types, chips) and color mapping into gameLogic.ts.
  // Why: Prevents recreating these static arrays and evaluating inline style strings on every re-render of the RouletteGame component. This avoids unnecessary memory allocation and garbage collection overhead.
  // Impact: Reduces re-renders workload and memory pressure, improving render performance and UI responsiveness during gameplay.
  const getNumberColor = (num: number) => {
    const color = ROULETTE_NUMBER_COLORS[num];
    return ROULETTE_COLOR_CLASSES[color];
  };
        ...
        <div className="flex gap-2">
          {ROULETTE_CHIPS.map(chip => (
            <button
              ...
              className={`w-12 h-12 rounded-full font-bold transition-all ${selectedChip === chip ? 'scale-110 ring-2 ring-yellow-300' : ''} ${ROULETTE_CHIP_COLOR_CLASSES[chip]}`}
            >
              {chip}
            </button>
          ))}
